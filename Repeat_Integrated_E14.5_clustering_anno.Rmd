...
# save metadata table
fl_samples_E14.5_12May_named_subset$barcode <- colnames(fl_samples_E14.5_12May_named_subset)
fl_samples_E14.5_12May_named_subset$UMAP_1 <- fl_samples_E14.5_12May_named_subset@reductions$umap@cell.embeddings[,1]
fl_samples_E14.5_12May_named_subset$UMAP_2 <- fl_samples_E14.5_12May_named_subset@reductions$umap@cell.embeddings[,2]
```  
...
Ery_E14.5_metadata <- fl_samples_E14.5_12May_named_subset@meta.data
...
fl_samples_E14.5_12May_named_subset@meta.data <- Ery_E14.5_metadata
```

```{r}
write.csv(fl_samples_E14.5_12May_named_subset@meta.data, file = paste0(E14.5_rnavelo, "Ery_E14.5_metadata.csv"), quote = F, row.names = F)
...
Ery_E14.5_counts_matrix <- GetAssayData(fl_samples_E14.5_12May_named_subset, assay = "SCT", slot = "counts")
writeMM(Ery_E14.5_counts_matrix, file = paste0(E14.5_rnavelo, "Ery_E14.5_counts.mtx"))
```

```{r}
# Write dimensionality reduction matrix
write.csv(fl_samples_E14.5_12May_named_subset@reductions$pca@cell.embeddings, file = paste0(E14.5_rnavelo, "Ery_E14.5_pca.csv"), quote = F, row.names = F)
```
...
#top_30_markers_fl_samples_E14.5_12May_named_subset_regressed <- markers_fl_samples_E14.5_12May_named_subset_regressed %>% group_by(cluster) %>% 
  #slice_max(avg_log2FC, n = 30)
```
...
```{r}
fl_samples_E14.5_12May_named_subset_regressed@active.ident <- factor(fl_samples_E14.5_12May_named_subset_regressed@active.ident, levels = c("MEP", "Mk", "EP1b", "EP2b", "EP1a", "EP2a", "EP3a", "EP4a", "EP5a", "PEBa", "PEBb", "PEBc"))
```


```{r, eval=FALSE}
fl_samples_E14.5_12May_named_subset_regressed$cluster_ident <- factor(fl_samples_E14.5_12May_named_subset_regressed$cluster_ident, levels = c("MEP", "Mk", "EP1b", "EP2b", "EP1a", "EP2a", "EP3a", "EP4a", "EP5a", "PEBa", "PEBb", "PEBc"))
```
...
```{r}
categories1 <- c("cellular respiration", "mitochondrial respiratory chain complex assembly", "protein folding", "response to endoplasmic reticulum stress", "translational elongation","electron transport chain", "rRNA metabolic process", "ribosome biogenesis", "cellular aldehyde metabolic process", "mRNA processing", "chromatin remodeling", "regulation of cellular response to oxidative stress", "cellular response to toxic substance")
clusterProfiler::dotplot(enrichedterm_res, showCategory = categories1) +
...
clusterProfiler::dotplot(enrichedreactome_res, showCategory = categories2) +
  theme(axis.text.x = element_text(face = "bold", size = 15)) +
theme(axis.text.y = element_text(face = "bold", size = 15)) +
theme(plot.title = element_text(size = 20, face = "bold")) +
theme(axis.title.x = element_text(size = 20, face = "bold")) +
...
dir.create("~/myproject/sc_analysis_project/msc_project/DE_analysis/data/FL_E12.5/")
dir.create("~/myproject/sc_analysis_project/msc_project/DE_analysis/results/FL_E12.5/")
dir.create("~/myproject/sc_analysis_project/msc_project/DE_analysis/figures/FL_E12.5/")
```
