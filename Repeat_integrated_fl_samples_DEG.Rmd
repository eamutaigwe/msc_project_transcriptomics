...
# Ideally, raw counts from the RNA slot should be used. But since our E10.5 replicates have serious batch effects, and because batch cannot be added to the design matrix due to sub-optimal experimental design during library prep, I used 'corrected' raw counts from the SCT slot. 
# I ran the analysis using raw counts from the RNA and SCT slots and compared the results. Although there are less genes obtained with the SCT slot, the results are comparable with those of the RNA slot.
...
EP_downgene_group1 <- readRDS("~/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP1/EP_downgene_group1.rds")
...
regulonActivity_bySample_group_Scaled <- regulonActivity_bySample_group_Scaled %>% as.data.frame() %>% rownames_to_column("TF") %>%  dplyr::filter(!(TF %in% c("Etv6 (700g)", "Rreb1 (522g)", "Esrra (908g)", "Zmiz1 (957g)", "Runx1 (19g)"))) %>% column_to_rownames("TF") %>% as.matrix()
                                        

tiff(file="/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP1/EP_downgene_group1_heatmap.tif", width = 800, height =800)
...
library(AUCell)
# Read in already saved scenicOptions object (computed on Compute Canada)
scenicOptions_EP2 <- readRDS("/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP2/int/scenicOptions_EP2.rds")
EP_upgene_group2_cellInfo <- readRDS("/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP2/int/EP_upgene_group2_cellInfo.rds")
scenicOptions_EP2@inputDatasetInfo$cellInfo <- "/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP2/int/EP_upgene_group2_cellInfo.rds"
scenicOptions_EP2@settings$dbDir <- "/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP2"
```
...
# Add sex column to cellInfo
# Add a column for joining
EP_upgene_group2_cellInfo <- EP_upgene_group2_cellInfo %>% dplyr::mutate(cells = rownames(EP_upgene_group2_cellInfo))
...
# Add sex column to cellInfo object
EP_upgene_group2_cellInfo <- left_join(EP_upgene_group2_cellInfo, fl_samples_DEG_noCd45_sex_meta, by = "cells")
EP_upgene_group2_cellInfo <- EP_upgene_group2_cellInfo %>% column_to_rownames("cells")
```


```{r}
# Save cellInfo
saveRDS(EP_upgene_group2_cellInfo, "/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP2/int/EP_upgene_group2_cellInfo_withsex.rds")
```
...
```{r}
# Add cellInfo to scenicOptions object
scenicOptions_EP2@inputDatasetInfo$cellInfo <- "/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP2/int/EP_upgene_group2_cellInfo_withsex.rds"
```



```{r}
setwd("/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP2")
set.seed(181)
regulons <- loadInt(scenicOptions_EP2, "regulons")

regulonAUC <- loadInt(scenicOptions_EP2, "aucell_regulonAUC")

# Extract AUC scores of regulons 
regulon_auc_score_EP2 <- getAUC(regulonAUC)

regulonAUC <- regulonAUC[onlyNonDuplicatedExtended(rownames(regulonAUC)),]
regulonActivity_bySample_group <- sapply(split(rownames(EP_upgene_group2_cellInfo), EP_upgene_group2_cellInfo$sample_grp),
                                     function(allcells) rowMeans(getAUC(regulonAUC)[,allcells]))
regulonActivity_bySample_group_Scaled <- t(scale(t(regulonActivity_bySample_group), center = T, scale=T))
...
annotation_col <-  data.frame(Cluster=unique(EP_upgene_group2_cellInfo$cluster_ident),
                              Timepoint=unique(EP_upgene_group2_cellInfo$sample_grp))
                              #Sex=EP_downgene_group1_cellInfo$sex)
annotation_colors <- list(Timepoint=cols_sample_group)

rownames(regulonActivity_bySample_group_Scaled) <- gsub("_extended", "", rownames(regulonActivity_bySample_group_Scaled))

regulonActivity_bySample_group_Scaled <- regulonActivity_bySample_group_Scaled %>% as.data.frame() %>% rownames_to_column("TF") %>%  dplyr::filter(!(TF %in% c("Cebpz (21g)", "Pml (298g)", "Gtf2f1 (85g)", "Hdac2 (212g)", "Ddit3 (10g)"))) %>% column_to_rownames("TF") %>% as.matrix()
                                        

#tiff(file="/Users/ekperekaamutaigwe/myproject/sc_analysis_project/msc_project/cisTarget_databases_EP2/EP_upgene_group2_heatmap.tif", width = 800, height =800)
heat1 <- ComplexHeatmap::Heatmap(regulonActivity_bySample_group_Scaled, name="Regulon act.",
                        cluster_columns = FALSE,
                        show_column_dend = FALSE,
                        cluster_column_slices = FALSE,
...
```{r}
fl_samples_DEG_noCd45_sex_proliferation_sex_known <- subset(fl_samples_DEG_noCd45_sex, subset = sex != "unknown")
rm(fl_samples_DEG_noCd45_sex)
```
...
DefaultAssay(fl_samples_DEG_noCd45_sex_proliferation_sex_known) <- "RNA"
fl_samples_DEG_noCd45_sex_proliferation_sex_known <- SCTransform(fl_samples_DEG_noCd45_sex_proliferation_sex_known,
                                   vst.flavor = "v2",
...
```{r}
fl_samples_DEG_noCd45_sex_proliferation_sex_known <- PrepSCTFindMarkers(fl_samples_DEG_noCd45_sex_proliferation_sex_known)
```


```{r}
VariableFeatures(fl_samples_DEG_noCd45_sex_proliferation_sex_known[["integrated"]]) <- rownames(fl_samples_DEG_noCd45_sex_proliferation_sex_known[["SCT"]]@scale.data)
```
...
```{r, eval=FALSE}
set.seed(12639)
DefaultAssay(fl_samples_DEG_noCd45_sex_proliferation_sex_known) <- "integrated"
# Run PCA
fl_samples_DEG_noCd45_sex_proliferation_sex_known <- RunPCA(object = fl_samples_DEG_noCd45_sex_proliferation_sex_known, verbose = FALSE)
```
...
```{r}
# Run UMAP
fl_samples_DEG_noCd45_sex_proliferation_sex_known <- RunUMAP(fl_samples_DEG_noCd45_sex_proliferation_sex_known, 
                  dims = 1:50,
...
# Plot UMAP
set.seed(123)
DimPlot(object = fl_samples_DEG_noCd45_sex_proliferation_sex_known,
        reduction = "umap",
        label = TRUE,
...
## Ery
```{r}
DefaultAssay(fl_samples_DEG_noCd45_sex_proliferation_sex_known) <- "SCT"
# Make a copy of the original object to avoid stories that touch
fl_samples_DEG_noCd45_sex_proliferation_sex_known1 <- fl_samples_DEG_noCd45_sex_proliferation_sex_known

# save metadata table
fl_samples_DEG_noCd45_sex_proliferation_sex_known1$barcode <- colnames(fl_samples_DEG_noCd45_sex_proliferation_sex_known1)
fl_samples_DEG_noCd45_sex_proliferation_sex_known1$UMAP_1 <- fl_samples_DEG_noCd45_sex_proliferation_sex_known1@reductions$umap@cell.embeddings[,1]
fl_samples_DEG_noCd45_sex_proliferation_sex_known1$UMAP_2 <- fl_samples_DEG_noCd45_sex_proliferation_sex_known1@reductions$umap@cell.embeddings[,2]
```  


```{r}
# Select column variables of interest
All_timepoints_metadata <- fl_samples_DEG_noCd45_sex_proliferation_sex_known1@meta.data
...
fl_samples_DEG_noCd45_sex_proliferation_sex_known1@meta.data <- All_timepoints_metadata
```


```{r}
write.csv(fl_samples_DEG_noCd45_sex_proliferation_sex_known1@meta.data, file = paste0(All_timepoints_rnavelo, "All_timepoints_metadata.csv"), quote = F, row.names = F)
```


```{r}
# Write expression counts matrix
All_timepoints_counts_matrix <- GetAssayData(fl_samples_DEG_noCd45_sex_proliferation_sex_known1, assay = "SCT", layer = "counts") 
...
write.csv(fl_samples_DEG_noCd45_sex_proliferation_sex_known1@reductions$pca@cell.embeddings, file = paste0(All_timepoints_rnavelo, "All_timepoints_pca.csv"), quote = F, row.names = F)
```
